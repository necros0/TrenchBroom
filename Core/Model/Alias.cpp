...
namespace TrenchBroom {
    ...
    AliasSingleFrame::AliasSingleFrame(string& name, vector<AliasFrameTriangle*>& triangles, TVector3f center, TBoundingBox bounds, TBoundingBox maxBounds) : name(name), triangles(triangles), center(center), bounds(bounds), maxBounds(maxBounds) {}
    ...
    : times(times), frames(frames) {
        ...
        if (frames.size() > 0) {
            ...
            for (int i = 1; i < frames.size(); i++)
                mergeBoundsWithBounds(&bounds, &frames[0]->bounds, &bounds);
        } else {
    ...
    TVector3f Alias::unpackFrameVertex(AliasPackedFrameVertex& packedVertex, TVector3f origin, TVector3f size) {
        TVector3f vertex;
    ...
    AliasSingleFrame* Alias::readFrame(istream& stream, TVector3f origin, TVector3f scale, int skinWidth, int skinHeight, vector<AliasSkinVertex>& vertices, vector<AliasSkinTriangle>& triangles) {
        ...
        vector<TVector3f> frameVertices(vertices.size());
        TVector3f center;
        TBoundingBox bounds;
