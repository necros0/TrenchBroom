...
namespace TrenchBroom {
    ...
    TVector3f Selection::center() const {
        TVector3f center;
        switch (m_mode) {
            case SM_FACES:
                center = m_faces[0]->center();
                for (int i = 1; i < m_faces.size(); i++) {
                    TVector3f temp = m_faces[i]->center();
            ...
            case SM_BRUSHES:
                ...
                for (int i = 1; i < m_brushes.size(); i++) {
                    TVector3f temp = m_brushes[i]->center();
                    addV3f(&center, &temp, &center);
                }
            ...
            case SM_ENTITIES:
                ...
                for (int i = 1; i < m_entities.size(); i++) {
                    TVector3f temp = m_entities[i]->center();
                    addV3f(&center, &temp, &center);
                }
    ...
    TBoundingBox Selection::bounds() const {
        TBoundingBox bounds;
        switch (m_mode) {
            case SM_FACES:
                bounds = m_faces[0]->brush()->bounds();
                for (int i = 1; i < m_faces.size(); i++) {
                    TBoundingBox temp = m_faces[i]->brush()->bounds();
            ...
            case SM_BRUSHES:
                ...
                for (int i = 1; i < m_brushes.size(); i++) {
                    TBoundingBox temp = m_brushes[i]->bounds();
                    mergeBoundsWithBounds(&bounds, &temp, &bounds);
                }
                break;
            case SM_ENTITIES:
                ...
                for (int i = 1; i < m_entities.size(); i++) {
                    TBoundingBox temp = m_entities[i]->bounds();
                    mergeBoundsWithBounds(&bounds, &temp, &bounds);
                }
                break;
            case SM_BRUSHES_ENTITIES:
