...
namespace TrenchBroom {
    ...
    BspFace::BspFace(BspTextureInfo* textureInfo, vector<TVector3f>& vertices) 
    ...
    TVector2f BspFace::textureCoordinates(TVector3f& vertex) {
        TVector2f result;
        result.x = (dotV3f(&vertex, &textureInfo->sAxis) + textureInfo->sOffset) / textureInfo->texture->width;
        result.y = (dotV3f(&vertex, &textureInfo->tAxis) + textureInfo->tOffset) / textureInfo->texture->height;
        return result;
    }

    BspModel::BspModel(vector<BspFace*>& faces, int vertexCount, TVector3f& center, TBoundingBox& bounds, TBoundingBox& maxBounds) 
    ...
    void Bsp::readVertices(istream& stream, int count, vector<TVector3f>& vertices) {
        TVector3f vertex;
        for (int i = 0; i < count; i++) {
            stream.read((char *)&vertex, sizeof(TVector3f));
    ...
    Bsp::Bsp(string& name, istream& stream) : name(name) {
        ...
        vertexCount = verticesLength / sizeof(TVector3f);
        
        vector<TVector3f> vertices;
